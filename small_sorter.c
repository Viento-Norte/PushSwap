...
/*   Updated: 2023/05/07 20:58:53 by legarcia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_sort_3(t_push_swap **pshswp)
{
	...
	while (!is_sorter((*pshswp)->stacka))
	{
		if (ft_maxnum((*pshswp)->stacka) == (*pshswp)->stacka->next->num)
			rra(pshswp);
		else if (ft_maxnum((*pshswp)->stacka) ==
			(*pshswp)->stacka->next->next->num)
			sa(pshswp);
	}
...
void sorter(t_push_swap	**pshswp)
{
	...
	if((*pshswp)->elmntsa== 2)
		...
		ft_sort_5(pshswp);
	else if (((*pshswp)->elmntsa > 5 ) && (*pshswp)->elmntsa < 400)
			ft_big_sorter(pshswp);
	else
		ft_giant_sorter(pshswp);	
}
